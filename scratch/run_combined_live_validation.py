...
def run_combined_validation():
    print("=================== STEP 3: COMBINED SYSTEM ADVERSARIAL VALIDATION ===================", flush=True)
    ...
    print(f"  Target: Cam0={len(cam0_target)}, Cam1={len(cam1_target)}, Cam2={len(cam2_target)}")
    print(f"  Hard Bystanders: Cam1={len(cam1_hard_bystanders)}, Cam2={len(cam2_hard_bystanders)}")
    
    # Selected Data-Derived Thresholds:
    # reacquisition_threshold = 0.85 (balanced against intra-camera stability)
    # reference_threshold = 0.78
    # upper_threshold = 0.75
    # Window size W = 5, min_consistency_ratio = 0.80 (requires 4 of 5 independent frames)
    # min_margin = 0.05
    ...
    for run_idx in range(n_runs):
        ...
        # 3. Target arrives on Camera 1 competing against Hard Bystander + brief occlusion
        target_reacquired_cam1 = False
        for f in range(20):
            t_crop = cam1_target[(run_idx + f) % len(cam1_target)]
            b_crop = cam1_hard_bystanders[(run_idx + f) % len(cam1_hard_bystanders)]
            
            # Simulate brief 2-frame occlusion at f=2, 3 where target crop is blurred/occluded
            if f in (2, 3):
            ...
            if dec.is_confirmed:
                if dec.best_track_id == 202:
                    ...
                    reacq_cam1_count += 1
                    # Enrolling verified Camera 1 viewpoint into Anchor!
    ...
    print("\n=================== LIVE TRIAL DIRECT OBSERVATION RESULTS ===================")
    ...
    print(f"False Bystander Adoptions:       {false_adoptions} (Target: 0)")
